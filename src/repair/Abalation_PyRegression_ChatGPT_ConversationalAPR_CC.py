...
class Abalation_PyRegression_ChatGPT_ConversationalAPR_CC(AbstractRepair):
    ...
    def _load_arguments(self, args):
        ...
        self.output_dir = os.path.join(args.output_dir, "conversational-apr-cc", self.dataset_name, self.model_name, self.data_id)
    ...
    def repair(self):
        ...
        for attempt in range(self.attempts):
            ...
            experiment_name = f"conversational-APR-CC_{self.model_name}_{self.dataset_name}_{self.data_id}_attempt-{attempt+1}-{self.attempts}_{self.marking_time}" # Name of the experiment
    ...
    # ================== FORMATTING QUERY ==================
    def _format_prompt(self, buggy_function: str, failedMessage: str, bug_inducing_changes: str, changed_files: list) -> list:
        system_prompt = "You are an expert Automatic Program Repair Tool specialized in fixing Python regression bugs."
        
        if bug_inducing_changes == "":
            instruction = """## Task\n
The following function contains a regression bug:\n{}

Bug-Inducing Context:
- The bug is NOT caused by direct edits inside this function.
- Instead, it originates from changes introduced in the bug-inducing commit.
- You MUST analyze the related file changes to infer the root cause.

Files modified in the bug-inducing commit that are likely related to the bug (identified via JaCoCo-based code coverage analysis):\n{}

Failing Test Cases and Error Messages:\n{}

## Instructions
Please think step by step to identify the root cause based on the bug-inducing context and their relationship to regression fixing patches. Provide the correct function.\n
...
"""
            messages = [
                ...
                {"role": "user", "content": instruction.format(buggy_function, ",".join(changed_files), failedMessage)},
            ]
        else:
            instruction = """## Task\n
...
Files modified in the bug-inducing commit:\n{}

Failing Test Cases and Error Messages:\n{}

## Instructions
Please think step by step to identify the root cause from the bug-inducing context and their relationship to regression fixing patches. Provide the correct function.\n
...
"""
            messages = [
                {"role": "developer", "content": system_prompt},
                {"role": "user", "content": instruction.format(buggy_function, bug_inducing_changes, ",".join(changed_files), failedMessage)},
            ]
        return messages
