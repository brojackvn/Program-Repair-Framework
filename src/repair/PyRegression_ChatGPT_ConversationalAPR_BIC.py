...
class PyRegression_ChatGPT_ConversationalAPR_BIC(AbstractRepair):
    ...
    def _load_arguments(self, args):
        ...
        self.output_dir = os.path.join(args.output_dir, "conversational-apr-bic", self.dataset_name, self.model_name, self.data_id)
    ...
    def repair(self):
        ...
        for attempt in range(self.attempts):
            ...
            experiment_name = f"conversational-APR-BIC_{self.model_name}_{self.dataset_name}_{self.data_id}_attempt-{attempt+1}-{self.attempts}_{self.marking_time}" # Name of the experiment
            ...
            for iteration in range(self.iterations):
                ...
                if iteration == 0:
                    messages = self._format_prompt(bug_info["buggy_function"], formatted_failed_message, bug_info["bug_inducing_changes"], bug_info["bic_commit_message"])
                else:
    ...
    # ================== FORMATTING QUERY ==================
    def _format_prompt(self, buggy_function: str, failedMessage: str, bug_inducing_changes: str, bic_commit_message: str) -> list:
